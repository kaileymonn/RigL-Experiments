...
def sparse_conv2d(x,
    ...
    with tf.variable_scope(
        ...
        if sparsity_technique in pruning_methods:
            ...
            raise ValueError('Unsupported sparsity technique {}'.format(sparsity_technique))
...
def sparse_fully_connected(x,
    ...
    with tf.variable_scope(name, 'Dense', [x], custom_getter=layer_variable_getter) as sc:
        ...
        if sparsity_technique in pruning_methods:
            return layers.masked_fully_connected(inputs=x,
                                                 num_outputs=units,
                                                 activation_fn=activation,
                                                 weights_initializer=kernel_initializer,
                                                 weights_regularizer=kernel_regularizer,
