...
def set_custom_sparsity_map():
    if FLAGS.first_layer_sparsity > 0.:
        CUSTOM_SPARSITY_MAP['resnet_model/initial_conv'] = FLAGS.first_layer_sparsity
    if FLAGS.last_layer_sparsity > 0.:
        CUSTOM_SPARSITY_MAP['resnet_model/final_dense'] = FLAGS.last_layer_sparsity
...
def train_function(training_method, loss, cross_loss, reg_loss, output_dir, use_tpu):
    ...
    metrics.update(utils.mask_summaries(masks[:4] + masks[-1:], with_img=FLAGS.log_mask_imgs_each_iteration))
...
def resnet_model_fn_w_pruning(features, labels, mode, params):
    ...
    def initialize_parameters_from_ckpt(ckpt_path):
        ...
        model_dir = FLAGS.output_dir
        already_has_ckpt = model_dir and tf.train.latest_checkpoint(model_dir) is not None
        if already_has_ckpt:
            tf.logging.info(
                'Training already started on this model, not loading masks from'
                'previously trained model')
    ...
    elif (FLAGS.mask_init_method and FLAGS.training_method not in ('snip', 'baseline')):
        def scaffold_fn():
            """For initialization, passed to the estimator."""
            if FLAGS.initial_value_checkpoint:
                initialize_parameters_from_ckpt(FLAGS.initial_value_checkpoint)
...
def main(argv):
    ...
    if FLAGS.num_eval_images % FLAGS.eval_batch_size != 0:
        raise ValueError('eval_batch_size (%d) must evenly divide num_eval_images(%d)!' %
